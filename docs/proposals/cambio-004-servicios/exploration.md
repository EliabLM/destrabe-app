...
- `backend/src/routes/services.routes.ts` _(nuevo)_ — `POST /services`, `GET /services/nearby`, `GET /services/:id`, `PATCH /services/:id/status`.
- `backend/src/routes/index.ts` — montar `servicesRouter`.
- `backend/src/controllers` _(nuevo, opcional)_ o handlers inline en el router — preferible controllers separados para no inflar el router (a decidir en design).
- `backend/src/services/serviceMachine.ts` _(nuevo)_ — máquina de estados pura (transiciones permitidas + quién puede dispararlas). Sin IO.
- `backend/src/middleware/validate.ts` _(nuevo)_ — `validate(schema, source)` factory Zod → 400 `{ error, code: 'VALIDATION_ERROR' }`. Reutilizable en futuros cambios.
- `backend/src/lib/queue.ts` _(nuevo)_ — cliente BullMQ (`Queue` + `Worker`) para el timer de 15 min `PENDING → CANCELLED`.
- `backend/src/jobs/serviceExpiry.job.ts` _(nuevo)_ — worker que procesa el job de expiración: carga el `Service`, si sigue `PENDING` → `CANCELLED` (+ futuro push, aquí solo log).
